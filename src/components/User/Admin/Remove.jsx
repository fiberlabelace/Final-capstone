...
const Remove = ({
  ...
  const handleRemoveCourse = course => {
    const confirmDelete = window.confirm(t("admin.confirmRemoveCourse", {
    ...
    if (isCustomCourse) {
      const updatedCustomCourses = customCourses.filter(item => item.maKhoaHoc !== course.maKhoaHoc);
    ...
    setCourses(prev => prev.filter(item => item.maKhoaHoc !== course.maKhoaHoc));
  ...
  return <div className="mt-5">
            ...
            <div className="row g-4">
                {courses.length > 0 ? courses.map(course => <div className="col-md-6 col-lg-4" key={course.maKhoaHoc}>
                            <div className="border rounded p-3 h-100">
                                <img src={course.hinhAnh || `https://picsum.photos/seed/${course.biDanh}/500/300`} alt={course.tenKhoaHoc} className="img-fluid rounded mb-3" onError={e => {
            e.currentTarget.src = `https://picsum.photos/seed/${course.biDanh}/500/300`;
                        ...
                        </div>) : <p>{t("admin.noCoursesAvailable")}</p>}
            </div>
        </div>;
};
