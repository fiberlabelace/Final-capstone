...
import { layDanhSachNguoiDung } from "../../API/users";
import Add from "./Add";
...
const Admin = () => {
  ...
  useEffect(() => {
    const loadCourses = async () => {
      try {
        ...
        const visibleApiCourses = apiCourses.filter(course => !removedCourses.includes(course.maKhoaHoc));
        setCourses([...visibleApiCourses, ...customCourses]);
      } catch (err) {
        console.log("COURSE STATUS:", err.response?.status);
        console.log("COURSE DATA:", err.response?.data);
      }
    };
    loadCourses();
  }, []);
  useEffect(() => {
    const loadUsers = async () => {
      try {
        ...
        const res = await layDanhSachNguoiDung("GP01");
        console.log("USERS API RESPONSE:", res.data);
        const apiUsers = res.data || [];
        const formattedUsers = apiUsers.map(account => ({
          name: account.hoTen,
          email: account.email,
          password: "",
          role: account.maLoaiNguoiDung,
        ...
        console.log("GET USERS STATUS:", err.response?.status);
        console.log("GET USERS DATA:", err.response?.data);
        if (err.response?.status === 401) {
          alert("401 Unauthorized: Bạn chưa đăng nhập bằng tài khoản CyberSoft có accessToken hợp lệ.");
        }
      }
    };
    loadUsers();
  }, []);
  if (!user || user.role !== "admin" && user.role !== "GV") {
  ...
  return <div>
            ...
            <div className="container py-5">
                ...
                {activeSection === "add" && <Add courses={courses} setCourses={setCourses} />}

                {activeSection === "remove" && <Remove courses={courses} setCourses={setCourses} />}

                {activeSection === "users" && <EditUser users={users} setUsers={setUsers} />}
