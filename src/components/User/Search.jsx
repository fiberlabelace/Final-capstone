    ...
    const loadCourses = async () => {
      ...
      try {
        ...
        const filteredCourses = allCourses.filter(course => {
          ...
          const description = normalizeText(course.moTa?.replace(/<[^>]*>/g, "") || "");
          const category = normalizeText(course.danhMucKhoaHoc?.tenDanhMucKhoaHoc || "");
          return courseName.includes(searchText) || description.includes(searchText) || category.includes(searchText);
        });
        const randomCourses = [...allCourses].sort(() => Math.random() - 0.5).slice(0, 6);
  ...
  return <div>
            ...
            <div className="container my-5">
                {loading ? <div className="text-center py-5">
                    ...
                    </div> : query.trim() && courses.length > 0 ? <>
                        <h1 className="mb-4">
                            {t("search.resultsFor", {
            query
          })}
                  ...
                  e.currentTarget.src = `https://picsum.photos/seed/${course.biDanh}/500/300`;
                                            ...
                                            <div className="card-body">
                                                ...
                                                <p className="card-text">
                                                    {course.moTa?.replace(/<[^>]*>/g, "").slice(0, 120) || t("course.noDescription")}
                                                </p>
                                            </div>
            ...
            color: "#FFC400"
              ...
              query
                        ...
                        {!query.trim() && <h1 className="text-center mb-5">
                                {t("search.interestedCourses")}
                            </h1>}
                        {relatedCourses.length > 0 && <>
                                {query.trim() && <h2 className="text-center mb-4">
                                        {t("search.relatedCourses")}
                                    </h2>}
                                <div className="row g-4">
                                    {relatedCourses.map(course => <div className="col-md-6 col-lg-4" key={course.maKhoaHoc}>
                                            <Link to={`/${course.biDanh}`} className="text-decoration-none">
                                                <div className="card h-100">
                                                    <img src={course.hinhAnh} className="card-img-top" alt={course.tenKhoaHoc} onError={e => {
                    e.currentTarget.src = `https://picsum.photos/seed/${course.biDanh}/500/300`;
