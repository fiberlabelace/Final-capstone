  ...
  return <div>
            <nav className="navbar navbar-expand-lg">
                <div className="container">
                    <Link to="/">
                        <img className="me-4" src={darkMode ? "/logo.png" : "/logo_dark.png"} style={{
            maxHeight: darkMode ? "50px" : "60px"
          }} />
                    </Link>
                    ...
                    <div className="collapse navbar-collapse justify-content-center" id="navbarSupportedContent">
                        <div className="position-relative d-flex align-items-center pe-5" style={{
            ...
            }} className="position-relative d-flex align-items-center pe-5" style={{
              width: "clamp(400px, 25vw, 400px)"
            }}>
                                <i className="bi bi-search position-absolute start-0 ms-3 text-secondary"></i>
                                <input type="search" className="form-control rounded-pill bg-light border-0 ps-5 py-1 shadow-none" placeholder={t("header.search")} aria-label={t("header.search")} value={searchQuery} onChange={e => setSearchQuery(e.target.value)} style={{
                color: "#6c6c6c"
              }} />
                            </form>
                        </div>
                        <ul className="navbar-nav ms-lg-auto mb-2 mb-lg-0 gap-2 text-center">
                            <li className="nav-item">
                                <button type="button" className="nav-link position-relative" data-bs-toggle="offcanvas" data-bs-target="#cartCanvas" aria-controls="cartCanvas">
                                    ...
                                    {t("header.cart")}
                                    {cartNotification > 0 && <span className="cart-notification">
                                            {cartNotification}
                                        </span>}
                                </button>
                            </li>
            ...
            <div className="offcanvas offcanvas-end" tabIndex="-1" id="cartCanvas" aria-labelledby="cartCanvasLabel">
                ...
                <div className="offcanvas-body">
                    <div className="cart-list d-flex flex-column gap-3">
                        {cart.length > 0 ? <>
                                {cart.map(course => <div key={course.biDanh}>
                        ...
                        {registeredCourses.length > 0 ? registeredCourses.map(course => <div key={course.biDanh}>
                                ...
                                </div>) : <p className="text-secondary mt-3">
