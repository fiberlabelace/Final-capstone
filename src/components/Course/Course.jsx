...
const Course = () => {
  ...
  useEffect(() => {
    axios.get("https://elearningnew.cybersoft.edu.vn/api/QuanLyKhoaHoc/LayDanhSachKhoaHoc?MaNhom=GP01", {
      ...
      const foundCourse = courses.find(item => item.biDanh === courseName);
      if (foundCourse) {
        setCourse(foundCourse);
      } else {
        setCourse({
          biDanh: courseName,
          tenKhoaHoc: courseName.split("-").map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(" "),
  ...
  useEffect(() => {
    ...
    const checkCourseStatus = () => {
      ...
      const alreadyInCart = cart.some(item => item.biDanh === course.biDanh);
      const alreadyRegistered = registeredCourses.some(item => item.biDanh === course.biDanh);
            ...
            <div className="course-page container my-5">
                <div className="row g-5 align-items-center">
                    <div className="col-lg-6">
                        <img src={course.hinhAnh} className="course-image img-fluid rounded-4" alt={course.tenKhoaHoc} onError={e => {
            e.currentTarget.src = `https://picsum.photos/seed/${course.biDanh}/800/500`;
          }} />
                    </div>
